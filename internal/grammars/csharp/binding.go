// Package csharp exposes the vendored csharp grammar. Its parse tables are
...
/*
#include "parser.h"
const TSLanguage *lint_tree_sitter_c_sharp(void);
...
// GetLanguage returns the Tree-sitter grammar for csharp. The first call
...
func GetLanguage() *sitter.Language {
	once.Do(func() {
		...
		C.lint_csharp_set_tables(C.CBytes(s[0]), C.CBytes(s[1]), C.CBytes(s[2]), C.CBytes(s[3]))
	})
	return sitter.NewLanguage(unsafe.Pointer(C.lint_tree_sitter_c_sharp()))
}
