// Package cpp exposes the vendored cpp grammar. Its parse tables are
...
/*
#include "parser.h"
const TSLanguage *lint_tree_sitter_cpp(void);
...
// GetLanguage returns the Tree-sitter grammar for cpp. The first call
...
func GetLanguage() *sitter.Language {
	once.Do(func() {
		...
		C.lint_cpp_set_tables(C.CBytes(s[0]), C.CBytes(s[1]), C.CBytes(s[2]), C.CBytes(s[3]))
	})
	return sitter.NewLanguage(unsafe.Pointer(C.lint_tree_sitter_cpp()))
}
