// Package tsx exposes the vendored tsx grammar. Its parse tables are
...
/*
#include "parser.h"
const TSLanguage *lint_tree_sitter_tsx(void);
...
// GetLanguage returns the Tree-sitter grammar for tsx. The first call
...
func GetLanguage() *sitter.Language {
	once.Do(func() {
		...
		C.lint_tsx_set_tables(C.CBytes(s[0]), C.CBytes(s[1]), C.CBytes(s[2]), C.CBytes(s[3]))
	})
	return sitter.NewLanguage(unsafe.Pointer(C.lint_tree_sitter_tsx()))
}
