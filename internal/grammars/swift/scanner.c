// Added by internal/grammars/gen: prefix the scanner's non-static
// globals so this package can link alongside the smacker original in
// the equivalence tests.
#define NON_CONSUMING_CROSS_SEMI_CHARS lint_swift_NON_CONSUMING_CROSS_SEMI_CHARS
#define OPERATORS lint_swift_OPERATORS
#define OP_ILLEGAL_TERMINATORS lint_swift_OP_ILLEGAL_TERMINATORS
#define OP_SYMBOLS lint_swift_OP_SYMBOLS
#define OP_SYMBOL_SUPPRESSOR lint_swift_OP_SYMBOL_SUPPRESSOR
#define RESERVED_OPS lint_swift_RESERVED_OPS
#define is_cross_semi_token lint_swift_is_cross_semi_token
...
void *lint_tree_sitter_swift_external_scanner_create() {
    return calloc(0, sizeof(struct ScannerState));
}

void lint_tree_sitter_swift_external_scanner_destroy(void *payload) {
    free(payload);
}

void lint_tree_sitter_swift_external_scanner_reset(void *payload) {
    struct ScannerState *state = (struct ScannerState *)payload;
    state->ongoing_raw_str_hash_count = 0;
}

unsigned lint_tree_sitter_swift_external_scanner_serialize(void *payload, char *buffer) {
    struct ScannerState *state = (struct ScannerState *)payload;
...
void lint_tree_sitter_swift_external_scanner_deserialize(
    void *payload,
    const char *buffer,
    unsigned length
) {
...
bool lint_tree_sitter_swift_external_scanner_scan(
    void *payload,
    TSLexer *lexer,
    const bool *valid_symbols
) {
