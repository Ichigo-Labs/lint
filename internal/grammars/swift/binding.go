// Package swift exposes the vendored swift grammar. Its parse tables are
...
/*
#include "parser.h"
const TSLanguage *lint_tree_sitter_swift(void);
void lint_swift_set_tables(const void *parse_table,
...
// GetLanguage returns the Tree-sitter grammar for swift. The first call
...
func GetLanguage() *sitter.Language {
	once.Do(func() {
		...
		C.lint_swift_set_tables(C.CBytes(s[0]), C.CBytes(s[1]), C.CBytes(s[2]), C.CBytes(s[3]))
	})
	return sitter.NewLanguage(unsafe.Pointer(C.lint_tree_sitter_swift()))
}
