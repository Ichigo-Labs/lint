# Vendored grammar: rust (compressed tables)

- Upstream grammar: github.com/tree-sitter/tree-sitter-rust v0.21.2
- Vendored from: github.com/smacker/go-tree-sitter@v0.0.0-20240827094217-dd81d9e9be82, directory rust/
...
- Generated 2026-06-10 by internal/grammars/gen. Regenerate with:
      ...
      go run ./internal/grammars/gen -sub rust -name rust -upstream "github.com/tree-sitter/tree-sitter-rust v0.21.2"

What gen changed relative to the smacker copy:

- ts_parse_table, ts_small_parse_table, ts_small_parse_table_map and
  ts_parse_actions (0.9 MB, ~95% of parser.c) are removed from the C source
  and shipped as tables.bin.gz; binding.go decompresses them on first use and
  hands them to lint_rust_set_tables(). The bytes are exactly what the
  C compiler laid out for the original arrays, captured by running a dumper
  built from the unmodified parser.c.
- Every public symbol is prefixed lint_ (lint_tree_sitter_rust, the external scanner
